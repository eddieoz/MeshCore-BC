...
struct DecapsulationResult {
  ...
  DecapsulationResult()
      : status(DecapsulationStatus::NOT_BITCHAT), messageId(0), fragmentNum(0), totalFragments(1),
        isFragment(false) {}
};
...
public:
   ...
   */
  bool decapsulate(const EncapsulatedPacket &packet, const uint8_t *channelKey, DecapsulationResult &result);
...
private:
  ...
  // Buffer for decryption to avoid stack overflow (~2KB)
  uint8_t _decryptionBuffer[BITCHAT_MAX_PAYLOAD_SIZE + BITCHAT_HEADER_SIZE];

  /**
   * Decrypt payload using channel key
   */
  void decryptPayload(const uint8_t *input, size_t len, const uint8_t *key, uint8_t *output) const;

  /**
   * Deserialize BitChat message from decrypted data
   */
  bool deserializeMessage(const uint8_t *data, size_t len, mesh::ble::BitchatMessage &msgOut);
};
