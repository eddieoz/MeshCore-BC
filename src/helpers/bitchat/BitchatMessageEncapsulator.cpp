...
BitchatMessageEncapsulator::BitchatMessageEncapsulator()
    : _nextMessageId(1)
{
}

bool BitchatMessageEncapsulator::encapsulate(const BitchatMessage& msg,
    ...
    size_t serializedLen = serializeMessage(msg, serialized, sizeof(serialized));
    if (serializedLen == 0) {
        return false;
    }
    ...
    // Build header
    uint8_t flags = 0;
    size_t headerLen = buildHeader(outPacket.data, sizeof(outPacket.data),
                                   static_cast<uint16_t>(serializedLen), flags,
                                   _nextMessageId++, 0, 1);
    ...
    // Encrypt payload
    encryptPayload(serialized, serializedLen, channelKey, 
                   &outPacket.data[headerLen]);
...
bool BitchatMessageEncapsulator::encapsulateWithFragmentation(
    const BitchatMessage& msg,
    const uint8_t* channelKey,
    ...
    size_t bytesPerFragment = BITCHAT_MAX_ENCAPSULATED_SIZE - BITCHAT_ENCAPSULATION_HEADER_SIZE - 6; // Extra header for frag info
    size_t offset = 0;
    ...
    for (size_t fragNum = 0; fragNum < fragCount; fragNum++) {
        ...
        size_t headerLen = buildHeader(pkt.data, sizeof(pkt.data),
                                       static_cast<uint16_t>(serializedLen), flags,
                                       msgId, static_cast<uint8_t>(fragNum), 
                                       static_cast<uint8_t>(fragCount));
        ...
        encryptPayload(&serialized[offset], thisFragmentSize, channelKey,
                       &pkt.data[headerLen]);
...
bool BitchatMessageEncapsulator::decapsulate(const EncapsulatedPacket& packet,
                                              const uint8_t* channelKey,
    ...
    decryptPayload(&packet.data[BITCHAT_ENCAPSULATION_HEADER_SIZE], payloadLen,
                   channelKey, decrypted);
    ...
    if (!packet.isFragment || totalFrags == 1) {
        ...
        msgOut.version = decrypted[offset++];
        msgOut.type = decrypted[offset++];
        msgOut.ttl = decrypted[offset++];
        
        // Timestamp (8 bytes, big-endian)
        msgOut.timestamp = 0;
        for (int i = 0; i < 8; i++) {
            msgOut.timestamp = (msgOut.timestamp << 8) | decrypted[offset++];
        }
        
        msgOut.flags = decrypted[offset++];
        
        // Payload length
        msgOut.payloadLength = (static_cast<uint16_t>(decrypted[offset]) << 8) | decrypted[offset + 1];
        offset += 2;
        
        // Sender ID (8 bytes, little-endian)
        uint64_t senderId = 0;
        for (int i = 0; i < 8; i++) {
            senderId |= (static_cast<uint64_t>(decrypted[offset + i]) << (i * 8));
        }
        ...
        if (msgOut.payloadLength > 0) {
            ...
            memcpy(msgOut.payload, &decrypted[offset], msgOut.payloadLength);
        }
...
size_t BitchatMessageEncapsulator::buildHeader(uint8_t* buffer, size_t bufferSize,
                                                uint16_t originalLength, uint8_t flags,
                                                uint32_t msgId, uint8_t fragNum, 
                                                uint8_t totalFrags) {
...
void BitchatMessageEncapsulator::encryptPayload(const uint8_t* input, size_t len,
                                                 const uint8_t* key,
                                                 uint8_t* output) {
...
void BitchatMessageEncapsulator::decryptPayload(const uint8_t* input, size_t len,
                                                 const uint8_t* key,
                                                 uint8_t* output) {
    // XOR is symmetric
...
size_t BitchatMessageEncapsulator::serializeMessage(const BitchatMessage& msg,
