...
bool SerialBLEInterface::addSecondaryService(BLEService &service) {
  ...
  service.begin();
  BLE_DEBUG_PRINTLN("SerialBLEInterface: Added secondary BLE service");
  return true;
}
...
void SerialBLEInterface::restartAdvertising(BLEService *secondaryService) {
  ...
  // Add the primary service (BLEUart) to advertising packet
  Bluefruit.Advertising.addService(bleuart);

  // Add device name to scan response
  Bluefruit.ScanResponse.addName();

  // Add secondary service to scan response if provided (advertising packet is too small for two 128-bit
  // UUIDs)
