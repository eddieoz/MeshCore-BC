...
void test_bitchat_service_adds_to_shared_server(void) {
  ...
  server.addService(MESHCORE_UART_SERVICE_UUID, std::make_unique<MeshCoreUARTService>(),
                    BLESecurityLevel::PIN_REQUIRED, 123456);

  // When: BitChat service added
  auto bitchatService = std::make_unique<BitchatBLEService>();
  bool added = server.addService(BITCHAT_SERVICE_UUID, std::move(bitchatService), BLESecurityLevel::OPEN);
  ...
  TEST_ASSERT_EQUAL_STRING("F47B5E2D-4A9E-4C5A-9B3F-8E1D2C3A4B5C", BITCHAT_SERVICE_UUID);

  // And: BitChat has OPEN security
  TEST_ASSERT_EQUAL(BLESecurityLevel::OPEN, server.getServiceSecurityLevel(BITCHAT_SERVICE_UUID));
...
void test_bitchat_service_has_open_security(void) {
  ...
  server.addService(BITCHAT_SERVICE_UUID, std::make_unique<BitchatBLEService>(), BLESecurityLevel::OPEN);
...
void test_bitchat_service_handles_message_type(void) {
  ...
  server.addService(BITCHAT_SERVICE_UUID, std::move(bitchatService), BLESecurityLevel::OPEN);

  server.simulateClientConnect(BITCHAT_SERVICE_UUID, /*conn_id=*/1);

  // When: BitChat MESSAGE received
  // Format: header(14) + sender(8) + payload(channel:text)
  uint8_t message[] = {                                                 // Header
                        0x01,                                           // version
                        0x02,                                           // type = MESSAGE
                        0x08,                                           // ttl
...
void test_bitchat_service_handles_announce_type(void) {
  ...
  PeerInfo peer;
  TEST_ASSERT_TRUE(bitchatPtr->getPeerById(0x123456789ABCDEF0ULL, peer));
