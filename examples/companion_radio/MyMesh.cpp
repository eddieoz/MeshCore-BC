...
#ifdef ENABLE_BITCHAT
  // Try BitChat handling first
  // Check for BC magic header at start (BitChat native) or after timestamp (legacy)
  bool isBitChat = false;
  size_t headerOffset = 0;

  if (len >= 2 && data[0] == 'B' && data[1] == 'C') {
    // BitChat format: [BC][version][payload...]
    isBitChat = true;
    headerOffset = 0;
    Serial.println("[BitChat] Detected BC magic at offset 0");
  } else if (len >= 6 && data[4] == 'B' && data[5] == 'C') {
    // Legacy format: [timestamp:4][BC][...]
    isBitChat = true;
    headerOffset = 4;
    Serial.println("[BitChat] Detected BC magic at offset 4 (legacy)");
  }

  if (isBitChat && _bitchatBridge != nullptr) {
    uint32_t timestamp = millis(); // Use current time as fallback
    Serial.printf("[BitChat] Processing packet, timestamp=%lu\n", timestamp);
    // Pass to BitChat bridge (skip BC header)
    // Pass to BitChat bridge (skip BC header ? No, pass everything)
    // Actually, decapsulate expects the FULL payload including BC header
    char senderName[32] = "mesh";
    _bitchatBridge->onMeshcoreEncapsulatedMessage(channel, timestamp, senderName, data, len);
  } else if (isBitChat) {
    Serial.println("[BitChat] WARNING: Bridge is null!");
  }
#endif
  // Default behavior: pass to base implementation
...
void MyMesh::loop() {
  // Call BitChat bridge loop FIRST (before BaseChatMesh::loop which may block)
#ifdef ENABLE_BITCHAT
  if (_bitchatBridge != nullptr) {
    _bitchatBridge->loop();
  }
...
// Static storage for found channel (since we need to return a pointer)
