...
void MyMesh::onChannelMessageRecv(const mesh::GroupChannel &channel, mesh::Packet *pkt, uint32_t timestamp,
                                  const char *text) {
#ifdef ENABLE_BITCHAT
  // Delegate BitChat forwarding to BitchatMesh subclass
...
MyMesh::MyMesh(mesh::Radio &radio, mesh::RNG &rng, mesh::RTCClock &rtc, SimpleMeshTables &tables, DataStore& store, AbstractUITask* ui)
    : mesh::bitchat::BitchatMesh(radio, *new ArduinoMillis(), rng, rtc, *new StaticPoolPacketManager(16), tables),
...
bool MyMesh::initBitchatMeshChannel() {
  ...
  return getBitchatBridge()->initMeshChannel();
}
#endif
